...
from shuup_boleto.constants import BankService
from shuup_boleto.models import StoredBoleto
from shuup_boleto.plugins import BoletoGeneratorCheckoutPlugin, BoletoInfoOrderHistoryPlugin
from shuup_tests.xtheme.test_plugins import get_context
...
def test_checkout_generator_plugin(rf):
    ...
    context = get_context(rf, reverse("shuup:order_complete", kwargs={"pk":31231, "key":"3123213"}))
...
def test_order_history_generator_plugin(rf):
    ...
    # order does not exist
    context = get_context(rf, reverse("shuup:show-order", kwargs={"pk":332312321}))
    assert plugin.get_context_data(context).get("order") is None
