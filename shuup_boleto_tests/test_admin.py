...
from shuup_boleto.admin.order_section import BoletoOrderSection
...
from shuup_boleto_tests.utils import get_payment_provider, initialize
...
def test_boleto_views_success(rf, admin_user):
    ...
    view = load("shuup.admin.views.dashboard.DashboardView").as_view()
    request = apply_request_middleware(rf.get("/"), user=admin_user)
    response = view(request)
    assert response.status_code == 200


    view = load("shuup_boleto.admin.views.DashboardView").as_view()
    request = apply_request_middleware(rf.get("/"), user=admin_user)
    response = view(request)
    assert response.status_code == 200
    ...
    # atribui como Pago
    view = load("shuup_boleto.admin.views.PayBoletoView").as_view()
    ...
    # limpa o status
    view = load("shuup_boleto.admin.views.ResetBoletoView").as_view()
    request = apply_request_middleware(rf.post("/"), user=admin_user)
    response = view(request, pk=order.pk)
    assert response.status_code == 200
    order.boleto.refresh_from_db()
    ...
    # cancela
    view = load("shuup_boleto.admin.views.CancelBoletoView").as_view()
    request = apply_request_middleware(rf.post("/"), user=admin_user)
    response = view(request, pk=order.pk)
    assert response.status_code == 200
    order.boleto.refresh_from_db()
